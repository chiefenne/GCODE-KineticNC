...
export const hoverDocs: Record<string, HoverDoc> = {
  ...
  "%": {
    ...
    syntax: "%",
    body: "A line with `%` as the first character marks the beginning of the NC program. All lines before it are ignored and treated like comments.",
  },
  ...
  G0: {
    ...
    notes:
      "Axes move as fast as possible to the specified coordinates. A specific path shape is not guaranteed.",
  ...
  G1: {
    ...
    notes:
      "If no F value is given in the current block, the last programmed feed rate remains active.",
  ...
  G2: {
    ...
    notes:
      "KinetiC-NC determines the arc plane automatically from the I/J/K parameters. Radius format with R is also supported. Helical interpolation is possible by adding the coordinate perpendicular to the arc plane.",
  ...
  G3: {
    ...
    notes:
      "Works like G2, but counterclockwise. Helical interpolation is possible in the same way as with G2.",
  ...
  G4: {
    ...
    example: "G4 H1.5",
    notes:
      "KinetiC-NC primarily uses H for dwell time in seconds. P is accepted for compatibility, but should be given in the same line. G4 can also be combined with UNTIL.",
  ...
  G20: {
    ...
    example: "G20",
    notes:
      "This changes the unit used for linear coordinates in the NC program. The screen display unit remains unchanged.",
  ...
  G28: {
    ...
    example: "G28",
    notes:
      "Moves to the configured parking or tool-change position in rapid traverse. If the parking Z is above the current position, Z is moved first to reduce collision risk.",
  ...
  G61: {
    ...
    example: "G61",
    notes:
      "Turns off edge rounding. Corner points are approached exactly, which usually requires braking to a stop and re-accelerating.",
  ...
  G62: {
    ...
    example: "G62",
    notes:
      "Turns on automatic corner rounding / polygon smoothing. The allowed path deviation depends on machine parameters.",
  ...
  G64: {
    ...
    example: "G64",
    notes:
      "Equivalent mode family to G62 for edge rounding / polygon smoothing.",
    body: "Enables Constant Velocity mode with smoothed path transitions.",
  },
  G90: {
  ...
  G17: {
    ...
    example: "G17",
    notes:
      "KinetiC-NC allows G17/G18/G19, but for G2/G3 the active arc plane is effectively inferred from the I/J/K parameters.",
  ...
  G40: {
    ...
    example: "G40",
    notes:
      "The first G1/G2/G3 move after G40 is the departure move. It starts from shifted coordinates and ends at unshifted coordinates, and should be at least one cutter radius long.",
  ...
  G41: {
    ...
    example: "G41",
    notes:
      "Offsets the tool path to the left in the direction of travel using the cutter radius from the tool list. The first G1/G2/G3 move after G41 is the lead-in move and should start outside the material and be at least one cutter radius long.",
  ...
  G42: {
    ...
    example: "G42",
    notes:
      "Offsets the tool path to the right in the direction of travel using the cutter radius from the tool list. The first G1/G2/G3 move after G42 is the lead-in move and should start outside the material and be at least one cutter radius long.",
  ...
  G43: {
    ...
    example: "G43",
    notes:
      "For subsequent motion commands, the current tool length from the tool list is added to the Z coordinate. The preview can be confusing because it always shows tool-tip motion.",
  ...
  G49: {
    ...
    example: "G49",
    notes:
      "After G49, no further tool length is added to subsequent Z coordinates.",
  ...
  G52: {
    ...
    example: "G52 Z0 A0",
    notes:
      "Equivalent to G74 in KinetiC-NC. Usually not needed because the controller ensures homing before the first program run after power-up. Selective homing is possible by specifying axes with zero coordinates; Z is always retracted first.",
  ...
  G74: {
    ...
    example: "G74 Z0 A0",
    notes:
      "Equivalent to G52 in KinetiC-NC. Usually not needed because the controller ensures homing before the first program run after power-up. Selective homing is possible by specifying axes with zero coordinates; Z is always retracted first.",
    body: "Starts a homing / reference run for all axes or for selected axes.",
  },
  G69: {
    ...
    example: "G69",
    notes:
      "The command is listed by the manual as cancelling the rotated coordinate system. Detailed behavior depends on whether such rotation is used in the machine setup or macros.",
  ...
  G73: {
    ...
    notes:
      "Implemented through a macro. In KinetiC-NC, G73 is predefined by default. It behaves like G83, but instead of fully retracting between steps, it only pauses briefly to break the chip.",
  ...
  G79: {
    ...
    example: "G79",
    notes:
      "Implemented through a machine-dependent macro, typically using a tool length sensor or touch-off switch. The manual shows an example where the tool is moved down until a switch input becomes active and the measured length is then written to the tool table.",
  ...
  G80: {
    ...
    example: "G80",
    notes:
      "Listed as optional in the manual. Used after a hole pattern to cancel the currently active drilling cycle.",
  ...
  G81: {
    ...
    example: "G81 Z-20 R5",
    notes:
      "Implemented through a macro and predefined by default in KinetiC-NC. Drills to depth Z at feed rate F, then retracts once to return height R.",
  ...
  G82: {
    ...
    notes:
      "Implemented through a macro and predefined by default in KinetiC-NC. Like G81, but includes a dwell time P at the bottom of the hole.",
  ...
  G83: {
    ...
    notes:
      "Implemented through a macro and predefined by default in KinetiC-NC. The drilling depth is split into steps of Q, with full retraction between steps for chip evacuation.",
  ...
  G84: {
    ...
    example: "G84 Z-12 R3",
    notes:
      "Not defined by default in KinetiC-NC. A suitable macro must be added for the machine.",
  ...
  M0: {
    ...
    example: "M0",
    notes:
      "Useful for test runs, manual checks, or operator confirmation between machining steps.",
  ...
  M2: {
    ...
    example: "M2",
    notes:
      "Terminates the current program and switches off active units such as spindle and coolant. Lines after M2 are not executed.",
  ...
  M4: {
    ...
    example: "M4 S6000",
    notes:
      "Only available if the machine supports reverse spindle rotation. If no S value is given, the last programmed spindle speed remains active.",
  ...
  M6: {
    ...
    example: "T3 M6",
    notes:
      "The exact behavior depends on machine parameters and whether manual or automatic tool change is configured. In practice, either M6 or M66 macro logic may be executed.",
  ...
  M8: {
    ...
    example: "M8",
    notes:
      "Depending on machine type, this usually switches on the first auxiliary unit, such as a coolant pump or chip extraction.",
  ...
  M9: {
    ...
    example: "M9",
    notes:
      "Switches off the first auxiliary unit and spray cooling if present. At M2 or M30, all units are switched off automatically even if M9 was omitted.",
  ...
  M14: {
    ...
    example: "M14",
    notes:
      "Used for tangential knife cutting to immerse the knife into the material.",
  ...
  M15: {
    ...
    example: "M15",
    notes:
      "Used for tangential knife cutting to lift the knife out of the material.",
  ...
  M30: {
    ...
    example: "M30",
    notes:
      "Like M2, but allows restart from the beginning. Useful for repeated production of identical parts.",
  ...
  M66: {
    ...
    example: "T3 M66",
    notes:
      "Forces a manual tool change even if the machine has an automatic tool changer.",
  ...
  M98: {
    ...
    example: "M98 P1234",
    notes:
      "Calls a subprogram identified by an O-number. KinetiC-NC also supports CALL as a more flexible alternative with file names and arbitrary identifiers.",
  ...
  M99: {
    ...
    example: "M99",
    notes:
      "Returns to the line after the last M98 in the main program. RETURN can also be used as an alternative.",
  ...
  IF: {
    ...
    syntax: "IF <condition> THEN <statement> [ELSE <statement>] ENDIF",
    example: [
      "IF #I100=1 THEN\n  M2(abort, no tool in the magazine)\nENDIF",
      "IF T<=10 THEN (tool no 1..10)\n  0 X-50 (first row of tool magazine)\nELSE (tool no. 11..20)\n  G0 X-100 (second row)\nENDIF",
    ],
    body: "Starts a conditional statement. Used together with a comparison and THEN, ELSE, ENDIF.",
  },
  THEN: {
    title: "THEN – Conditional execution",
    syntax: "IF <condition> THEN <statement> [ELSE <statement>] ENDIF",
    example: [
      "IF #I100=1 THEN\n  M2(abort, no tool in the magazine)\nENDIF",
      "IF T<=10 THEN (tool no 1..10)\n  0 X-50 (first row of tool magazine)\nELSE (tool no. 11..20)\n  G0 X-100 (second row)\nENDIF",
  ...
  ELSE: {
    title: "ELSE – Conditional execution",
    syntax: "IF <condition> THEN <statement> ELSE <statement> ENDIF",
    example: [
      "IF #I100=1 THEN\n  M2(abort, no tool in the magazine)\nENDIF",
      "IF T<=10 THEN (tool no 1..10)\n  0 X-50 (first row of tool magazine)\nELSE (tool no. 11..20)\n  G0 X-100 (second row)\nENDIF",
  ...
  ENDIF: {
    title: "ENDIF – Conditional execution",
    syntax: "IF <condition> THEN <statement> [ELSE <statement>] ENDIF",
    example: [
      "IF #I100=1 THEN\n  M2(abort, no tool in the magazine)\nENDIF",
      "IF T<=10 THEN (tool no 1..10)\n  0 X-50 (first row of tool magazine)\nELSE (tool no. 11..20)\n  G0 X-100 (second row)\nENDIF",
  ...
  UNTIL: {
    ...
    notes:
      "In KinetiC-NC this is currently limited to simple conditions using a single input signal.",
  ...
  "#0": {
    ...
    example: 'ASKINT "Enter count" I=1 J=10\n#100=#0',
    notes:
      "ASKBOOL, ASKINT, and ASKFLT use #0 to return the user response or entered value.",
  ...
  "#900": {
    ...
    notes:
      "Part of the current zero-point memory image used together with #901..#908 and #909.",
  ...
  "#901": {
    ...
    notes:
      "Part of the current zero-point memory image used together with #900 and #902..#909.",
  ...
  "#902": {
    ...
    example: "#902=#0-#4",
    notes:
      "Often used in probing macros to modify the Z value of the active offset before saving it with L53.",
  ...
  "#909": {
    ...
    example: "#7=#909\nG53\n...\n#909=#7",
    notes:
      "0 = G53, 1 = G54, and so on. The manual shows saving #909 before switching to G53 and restoring it afterwards in macros.",
